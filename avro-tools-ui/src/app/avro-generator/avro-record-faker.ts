...
//@ts-ignore
import * as mersenne from 'mersenne-twister';
...
function generateRecord(avroSchema, context) {
  ...
  console.log(avroSchema, context.registry)
...
export function Seeded<T = any>(seed: number) {
  ...
  const uuid = uuid4.bind(null);

  return (schema: avsc.AvroSchema, options: Options = {}): T =>
    generateAvroRecord(schema, {
      ...options,
      generators: {
        uuid: () => uuid().toString(),
        random,
...
export default function generateAvroRecord<T = any>(
  schema: avsc.AvroSchema,
  options: Options = {},
): T {
